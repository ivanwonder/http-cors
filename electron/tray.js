...
const {_isMacintosh} = require('../utils/platform')
...
ipc.on('put-in-tray', function (event) {
  ...
  const iconPath = path.join(!isDev ? app.getAppPath() : path.resolve(__dirname), './resource/trayImage/' + iconName)
...
if (!shareprocess) {
  ...
  shareprocess.loadURL(url.format({
    pathname: path.join(!isDev ? app.getAppPath() : path.resolve(__dirname), './resource/shareprocess.html'),
    protocol: 'file:'
  }))
